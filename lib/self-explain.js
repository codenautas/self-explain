...
selfExplain.assert.differences = function differences(a, b, opts){
    opts = opts || {};
    if(a === b){
        return null;
    }
    ...
    if(typeof a === "string" && typeof b === "string" && (opts.split.test(a) || opts.split.test(b))){
        ...
        for(var i=0; i<Math.max(aParts.length, bParts.length); i++){
            var dif = selfExplain.assert.differences(aParts[i]||'', bParts[i]||'', opts);
            if(dif){
...
selfExplain.assert.alldifferences = function bigdifferences(a,b){
    return selfExplain.assert.differences(a, b, selfExplain.assert.alldifferences.opts);
}
...
selfExplain.assert.bigdifferences = function bigdifferences(a,b){
    return selfExplain.assert.differences(a, b, selfExplain.assert.bigdifferences.opts);
}

selfExplain.assert.bigdifferences.opts = changing(selfExplain.assert.alldifferences.opts,{
    autoTypeCast:true,
    delta:0.00000001,
    split: /\r?\n/,
});
