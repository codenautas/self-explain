    ...
    previousResult.parts.forEach(function(part){
        ...
        if(part.type){
            ...
            nodeDef.childProperties.forEach(function(childProperty){
                ...
                }else if(childProperty.name){
                    newParts.push(JSON.stringify(part[childProperty.name].name||part[childProperty.name].type));
                }else if(childProperty.onlyNode){
                    newParts.push(part[childProperty.onlyNode]);
                }else if(childProperty.node){
                    parenthesis(newParts, part[childProperty.node], childProperty.punctuator||part[childProperty.operator], !!childProperty.left);
...
function timeStr(dt) { return date.ymdHmsMString(dt).substr(11); }
...
selfExplain.assert.differences = function differences(a, b, opts, prefix){
    ...
    if(a instanceof Date && b instanceof Date) {
        ...
        if(date.ymdString(a) !== date.ymdString(b)) {
            var aas = date.ymdHmsMString(a);
            var bbs = date.ymdHmsMString(b);
    ...
    if(a instanceof Object && b instanceof Object){
        var constructorName = function constructorName(o){
            var cn = o.constructor.name;
            if(!cn){
                return o.constructor.toString().replace(/^\s*function\s*([^(]*)\((.|\s)*$/i,'$1');
            }
            return cn;
        }
