...
selfExplain.assert.differences = function differences(a, b, opts, prefix){
    ...
    if(a instanceof Date && b instanceof Date) {
        ...
        var aa = new Date(a.getTime()-tzOffset);
        var bb = new Date(b.getTime()-tzOffset);
        var rv = [];
        var aas = aa.toISOString();
        var bbs = bb.toISOString();
        var timesDiffer = timeStr(a) !== timeStr(b);
        var maxDiff = 359999000; // 99 * 60 * 60 * 1000 + 59 * 60 * 1000 + 59 * 1000;
        var timeDiff = new Date(aa.getTime()-bb.getTime());
        if(dateStr(a) !== dateStr(b)) {
            rv.push(aas.slice(0, 10));
            if(timesDiffer) {
                rv.push(' ');
                rv.push(aas.substr(11, aa.getMilliseconds()?12:8));
            }
            rv.push(' != ');
            rv.push(bbs.slice(0, 10));
            if(timesDiffer) {
                rv.push(' ');
                rv.push(bbs.substr(11, bb.getMilliseconds()?12:8));
            }
        }
        if(timesDiffer && timeDiff <= maxDiff) {
            // console.log("aa", aa);
            // console.log("bb", bb);
            // console.log("timeDiff", timeDiff)
            // console.log("timeDiff", timeStr(timeDiff))
            rv.push(timeDiff.toISOString().substr(11, timeDiff.getMilliseconds()?12:8));
        }
        return rv.join('');
    }
    if(a instanceof Object && b instanceof Object){
        ...
        for(var name in a){
            if(!opts.unordered && bKeys[i]!==name){
                if(!collect(prefix+'{'+i+'}: .'+name+' != '+(i>=bKeys.length?'undefined':'.'+bKeys[i]))) break;
            }else{
        ...
        if(opts.unordered){
            ...
            while(i<bKeys.length){
                if(!collect(prefix+'{'+i+'}: undefined != .'+bKeys[i])) break;
                i++;
            }
        }
        return result.length ? result.join("\n") : null;
