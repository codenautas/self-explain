...
function timeStr(dt, withMS) {
    return dt.getHours()+dt.getMinutes()+dt.getSeconds()+(withMS?dt.getMilliseconds():'');
}
...
selfExplain.assert.differences = function differences(a, b, opts, prefix){
    ...
    if(a instanceof Date && b instanceof Date) {
        ...
        if(dateStr(a) === dateStr(b)) {
            return new Date(aa.getTime()-bb.getTime()).toISOString().substr(11, 8);
        }
        ...
        var sliceStart = 0;
        var sliceEnd = 10;
        if(timeStr(a) != timeStr(b)) {
            aas = replaceAt(aas, 10, ' ');
            bbs = replaceAt(bbs, 10, ' ');
            sliceEnd = 19;
        }
        return aas.slice(sliceStart, sliceEnd)+' != '+bbs.slice(sliceStart, sliceEnd);
    }
